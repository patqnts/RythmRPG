...
public class FlatKitFog : ScriptableRendererFeature {
    ...
    public override void Create() {
        // Material updates can re-enter Create; stop the recursion.
    ...
    private void CreateInternal() {
        ...
        {
#if UNITY_EDITOR
            var effectMaterial = SubAssetMaterial.GetOrCreate(settings, ShaderName);
            if (effectMaterial == null) return;
            settings.effectMaterial = effectMaterial;
#endif
    ...
    private void UpdateDistanceLut() {
        ...
        _lutDepth.Apply();
        if (_effectMaterial.GetTexture(distanceLut) != _lutDepth) {
    ...
    private void UpdateHeightLut() {
        ...
        _lutHeight.Apply();
        if (_effectMaterial.GetTexture(heightLut) != _lutHeight) {
