...
// Unity 6.1 renamed the Forward+ light loop macros. Keep one code path for all supported URP versions.
#if UNITY_VERSION >= 60001000
#define FLATKIT_USE_CLUSTER_LIGHT_LOOP USE_CLUSTER_LIGHT_LOOP
#define FLATKIT_CLUSTER_LIGHT_LOOP_SUBTRACTIVE_LIGHT_CHECK CLUSTER_LIGHT_LOOP_SUBTRACTIVE_LIGHT_CHECK
#else
#define FLATKIT_USE_CLUSTER_LIGHT_LOOP USE_FORWARD_PLUS
#define FLATKIT_CLUSTER_LIGHT_LOOP_SUBTRACTIVE_LIGHT_CHECK FORWARD_PLUS_SUBTRACTIVE_LIGHT_CHECK
#endif
...
inline half NdotLTransitionTexture(half3 normal, half3 lightDir, sampler2D stepTex) {
    ...
    const half4 rampColor = tex2Dlod(stepTex, half4(angleDiff, 0.5, 0, 0));
...
half4 UniversalFragment_DSTRM(InputData inputData, SurfaceData surfaceData, float2 uv)
{
    ...
    */
    ...
    #if FLATKIT_USE_CLUSTER_LIGHT_LOOP
    for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++)
    {
        FLATKIT_CLUSTER_LIGHT_LOOP_SUBTRACTIVE_LIGHT_CHECK
